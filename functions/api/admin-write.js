...
export async function onRequest(context) {
  ...
  const { ADMIN_PIN, GITHUB_TOKEN, GITHUB_REPO, GITHUB_BRANCH } = env;
  if (!ADMIN_PIN || !GITHUB_TOKEN || !GITHUB_REPO) {
    return jsonResponse(500, { error: 'Config serveur incomplète (env vars manquantes)' });
  }
  ...
  if (String(body.pin || '') !== String(ADMIN_PIN)) {
    return jsonResponse(401, { error: 'PIN invalide' });
  }
  ...
  if (!fn || !filePath) return jsonResponse(400, { error: `Action inconnue: ${action}` });
